    ...
    void WasmLibrary::WasmLoadImports(Wasm::WasmModule * wasmModule, ScriptContext* ctx, Var* importFunctions, Var moduleEnv, Var ffi)
    {
        ...
        for (uint i = 0; i < wasmModule->GetImportGlobalCount(); i++) 
        {
            ...
            case Wasm::WasmTypes::F32:
            {
                ...
                {
                    throw Wasm::WasmCompilationException(_u("Import global %s.%s (%d) must be f32"), global->importVar->modName, global->importVar->fnName, i);
                }
            ...
            case Wasm::WasmTypes::F64:
            {
                if (!JavascriptNumber::Is(prop))
                {
                    throw Wasm::WasmCompilationException(_u("Import global %s.%s (%d) must be v64"), global->importVar->modName, global->importVar->fnName, i);
                }
